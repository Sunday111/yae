...
class ResolvedSettings:
    ...
    def from_project(cls, project_root: Path, cli_cloned_repositories_dir: Path | None = None) -> "ResolvedSettings":
        ...
        cloned_repositories_dir, source = cls._select_cloned_repositories_dir(project_root, cli_cloned_repositories_dir)
    ...
    @staticmethod
    def _select_cloned_repositories_dir(project_root: Path, cli_value: Path | None) -> tuple[Path, str]:
        """First present source wins: CLI → local-config → environment → project default."""
        if cli_value is not None:
            return cli_value, "cli"
        if local_value := _read_local_cloned_repositories_dir(project_root):
            return _resolve_project_path(project_root, local_value), "local-config"
        if env_value := os.environ.get(CLONED_REPOSITORIES_DIR_ENV):
            return Path(env_value), "environment"
        return project_root / yae_constants.CLONED_REPOSITORIES_DIRECTORY_NAME, "default"
